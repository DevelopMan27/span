			   ...
			   std::string &qr_code_string)
{
	...
	for (int i = 0, k = 0; i < final_numbers.size(); i++)
	{
		for (int j = 0; j < final_numbers[i].second.size(); j++)
		{
			...
			snprintf(add_c, 6, "%lx", final_numbers[i].second[j]);
		...
		if (final_numbers[i].first < MAX_SUM_VALUE)
		{
			...
			else
			{
				...
				snprintf(color_hex, 10, "%lx", final_numbers[i].first);
					   ...
					   long max_serial_lenght)
{
	...
	snprintf(c, 10, "%ld", hardware_index);

	output.insert(output.begin(), c[0]);

	//    _itoa_s(validity_id, c, 10, 10);
	snprintf(c, 10, "%lx", validity_id);
	output.insert(output.begin(), c[0]);
				  ...
				  std::vector<std::string> &color_hex_value2)
{
	...
	for (int i = 0; i < renew_ids.size(); i++)
	{
		...
		if (renew_ids[i].id_type == CAM_INDEX)
		{
			...
			snprintf(c, 10, "%ld", renew_ids[i].station_id);
...
std::string getDataFromQr(std::string string1)
{
	...
	lic_info.append("\nPRODUCT NO : " + getProductNumber(product_id.id));
	...
	if (renew_ids.size() > 0)
	{
		...
		for (int i = 0; i < renew_ids.size(); i++)
		{
			...
			{
				lic_info.append("\nPRODUCTdsfgdfbg : " + std::to_string(renew_ids[i].validity) + " " +
								renew_ids[i].id + " " + renew_ids[i].date);
				if (renew_ids[i].validity == PART_OK)
				{
					product_code = renew_ids[i].id;
				}
				lic_info.append("\nPRODUCT NO --- : " + renew_ids[i].date + "$" + getProductNumber(renew_ids[i].id));
			}
...
//     static const std::string staticData = "252-252-252-252-252-252-252-252-252-252-252-252-5c6-16a-96-f3f-43f-a05-718-da5-718-c38-252-252-252-252-252-252-65d-886-45b-dd1-3e7-f36-f3f-f2a-683-56f-d06-e4b-56b-609-aee-b1f-bff-bff-bff-f7c-935-30-fcd-f33-f33-f33-662-ea3-662-910-718-3bd-ef2-ee9-ee9-1c5-21f-9c2-252-252-252-252-252-252-252-252-65d-f36-886-65d-dd1-b29-c05-f36-f36-c05-741-85f-d35-43f-16000-";

//     // Process the QR data (or perform any operation with the static string)
//     std::string result = getDataFromQr(staticData);
