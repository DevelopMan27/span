...
const getListOfAllCat = async () => {
  try {
    ...
    const response = await fetch(
      `https://hum.ujn.mybluehostin.me/span/v1/company_info_all.php`,
      {
        method: "POST",
        headers: {
    ...
    const formattedData = result.data.map(({ company_full_name, id }) => ({
...
export const AddCompLenseSerial = ({
    ...
    <>
      <View style={styles.container}>
        ...
        <CustomTextInput
          ...
          placeholder="Enter computer lense serial"
          value={updatedName}
          onChangeText={(text) => {
            setUpdatedName(text);
          }}
